...
// ─────────────────────────────────────────────────────────────────────────────
function getApiKeys(req?: Request, body?: any): string[] {
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (keys.length === 0) {
      console.error(`[Proposal:${requestId}] ❌ No API keys configured in environment or request.`);
      return NextResponse.json(
        {
          error: 'NO_API_KEY',
          message: 'Service is not configured. Please add your Gemini API Key in Settings or set GOOGLE_API_KEY in .env.local.'
        },
        { status: 503 }
      );
    }
