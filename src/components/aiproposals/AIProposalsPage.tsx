...
// ─── Main Component ───────────────────────────────────────────────
// Preload image as base64 to fix CORS/canvas taint issues in PDF export
...
export default function AIProposalsPage() {
  ...
  const [sigBase64, setSigBase64] = useState<string>('/ceo-signature.png');
  const [logoBase64, setLogoBase64] = useState<string>('/logo1.png');
  ...
  // Initialize quota + preload images as base64 on mount
  useEffect(() => {
    const q = getQuota();
    setQuotaUsed(q.used);
    ...
    // Preload signature & logo as base64 to avoid canvas taint in PDF export
  ...
  const downloadPDF = async () => {
    ...
    if (!element || isPdfDownloading) return;
    ...
    try {
      ...
      pdf.save(`${prompt.clientName || 'Proposal'}.pdf`);
    } catch (err) {
      console.error(err);
          ...
          <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50 rounded-t-xl">
            ...
            {generatedProposal && (
              <div className="flex gap-2">
                ...
                <button onClick={downloadPDF} disabled={isPdfDownloading} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-md transition-colors font-bold text-sm text-indigo-600 dark:text-indigo-400 disabled:opacity-50" title="Download PDF">{isPdfDownloading ? '...' : 'PDF'}</button>
                <button
            ...
            {isGenerating ? (
              ...
              <div id="proposal-pdf-content" className="space-y-6 text-slate-700 dark:text-slate-300 proposal-content bg-white p-8 rounded-xl print-container">
                <div className="flex justify-between items-start border-b-2 border-slate-200 dark:border-slate-800 pb-6 mb-6">
                  ...
                  <img src={logoBase64} alt="DevCodeX Logo" className="h-16 object-contain" crossOrigin="anonymous" />
