    ...
    try {
      ...
      if (imgH <= pageH) {
        ...
        while (yOffset < canvas.height) {
          ...
          pdf.addImage(sliceData, 'JPEG', 0, 0, pageW, pageW * (sliceH / canvas.width), undefined, 'FAST');
          yOffset += sliceH;
          firstPage = false;
        }
      }
    ...
    <div className="min-h-screen p-3 sm:p-4 lg:p-8 space-y-4 sm:space-y-6 max-w-[1700px] mx-auto animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
        ...
        <button
          ...
          className="btn-primary shrink-0 py-2.5 px-5 sm:py-3 sm:px-6 shadow-indigo-500/25 text-sm sm:text-base w-full sm:w-auto flex items-center justify-center gap-2 disabled:opacity-60"
        >
          <Download size={16} />
