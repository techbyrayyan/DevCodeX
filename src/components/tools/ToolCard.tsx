...
const ToolCard = memo(({ tool, index = 0, viewMode = 'grid', onClick, getColorClasses }: ToolCardProps) => {
  const [isClicked, setIsClicked] = useState(false);
  const Icon = tool.icon || Sparkles;

  const getColors = getColorClasses || ((color: string) => {
    const map: Record<string, { text: string; lightBg: string; border: string; hoverBorder: string }> = {
  ...
  const handleClick = (e: any) => {
    ...
    // Immediately trigger click glow animation
    setIsClicked(true);
    setTimeout(() => setIsClicked(false), 700);

    // Delay popup open so animation plays first
  ...
  return (
    // Outer wrapper with overflow:visible so shockwave rings expand beyond card boundary
    <div className="relative" style={{ isolation: 'isolate' }}>

      {/* The actual card — overflow:hidden so internal content is clipped cleanly */}
      <motion.div
      ...
      >
        {/* Inner clicked glow — stays inside card, adds warmth */}
        <AnimatePresence>
          {isClicked && (
            <motion.div
              key="inner-glow"
      ...
      {/* Shockwave rings - rendered OUTSIDE card so overflow:hidden doesn't clip them */}
      <AnimatePresence>
        {isClicked && (
          <>
            {/* Ring 1 - fast expand */}
