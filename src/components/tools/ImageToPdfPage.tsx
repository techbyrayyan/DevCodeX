  ...
  const processPdfToWord = async (file: File) => {
    try {
      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        ...
        const viewport = page.getViewport({ scale: 1.5 }); // higher scale = better quality
        ...
        // ── Calculate A4 dimensions in EMU (English Metric Units) ──────────
        // A4 width = 210mm → in EMU at 96dpi ≈ 8229600 EMU (but we use 6858000 for margins)
        const maxWidthEmu = 6858000; // ~180mm usable width
        const aspectRatio = viewport.height / viewport.width;
        const imgWidthEmu = maxWidthEmu;
        const imgHeightEmu = Math.round(maxWidthEmu * aspectRatio);

        // ── Add page image as a paragraph ─────────────────────────────────
        docChildren.push(
          new docx.Paragraph({
            children: [
              new docx.ImageRun({
                data: bytes,
                transformation: {
                  width: Math.round(imgWidthEmu / 9525),  // EMU to points (1pt = 9525 EMU)
        ...
        // ── Also extract text so it's fully readable & editable in Word ───────
        const textContent = await page.getTextContent();
        const pageText = textContent.items.map((item: any) => item.str).join(' ').trim();
        if (pageText) {
          docChildren.push(
            new docx.Paragraph({
              children: [new docx.TextRun({ text: pageText, size: 22, color: '111827' })], // 11pt readable & editable text
