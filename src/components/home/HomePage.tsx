...
import WelcomePopup from '@/components/tools/WelcomePopup';
import {
...
export default function HomePage() {
  const { goTo } = useAuth();
  const popupNav = usePopupNavigation((toolId: any) => goTo(toolId));
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-white overflow-x-hidden">
        ...
        <div className="relative max-w-5xl mx-auto space-y-8" style={{ animation: 'fadeUp 0.8s ease-out both' }}>
          ...
          <h1 className="text-3xl sm:text-5xl md:text-7xl font-extrabold tracking-tight leading-tight">
            ...
            <span className="relative">
              ...
              <span className="inline-block w-1 h-8 sm:h-14 md:h-20 bg-indigo-500 ml-1 align-middle animate-pulse rounded-sm" />
            </span>
          </h1>
        ...
        <FadeSection className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {[
              ...
              <button
                key={tool.id}
                onClick={(e) => popupNav.handleToolClick(e, tool.id, tool)}
      ...
      {/* Welcome Popup Animation before opening tool */}
      <WelcomePopup
        isOpen={popupNav.isOpen}
        onContinue={popupNav.handleContinue}
        toolName={popupNav.selectedTool?.label}
      />
    </div>
  );
}
