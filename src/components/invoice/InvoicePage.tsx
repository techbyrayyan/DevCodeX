...
export default function InvoicePage() {
  ...
  // PDF download — fast approach using optimized html2canvas + jsPDF
  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      // Render at scale 1 for speed (looks great at A4)
      const canvas = await html2canvas(el, {
        scale: 1.5,           // balanced speed vs quality
        useCORS: true,
        allowTaint: false,
      ...
      if (imgH <= usableH) {
        ...
        while (yOffset < canvas.height) {
          ...
          const sliceImgH = usableW * (sliceH / canvas.width);
          if (!firstPage) pdf.addPage();
          pdf.addImage(sliceData, 'JPEG', margin, margin, usableW, sliceImgH, undefined, 'FAST');
          yOffset += sliceH;
          firstPage = false;
        }
      }
  ...
  return (
    <div className="flex min-h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <aside className="w-56 bg-white dark:bg-gray-800 shadow-inner p-4 space-y-3 flex-shrink-0">
        ...
        <button onClick={downloadPdf} disabled={isDownloading} className="w-full bg-accent hover:bg-green-600 disabled:opacity-60 disabled:cursor-not-allowed text-white rounded-md py-2 text-sm transition-all hover-lift flex items-center justify-center gap-2">
          {isDownloading ? (
            <>
